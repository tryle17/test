...
OPTIONS:
    ...
    -p, --platform
        Specifies platform(SM8750/SM8650)
    -b, --build
        ...
        techpack dtbs：vendor/qcom/proprietary/display-devicetree camera-devicetree vendor/qcom/opensource/camera-devicetree
        base dtbs: sun.dtb sunp-v2.dtb dodge-23821-sun-overlay.dtbo petrel-24001-sun-overlay.dtbo
Usage:
    1 ./kernel_platform/oplus/build/oplus_build.sh -t user -p SM8750 -b kernel
    仅修改kernel代码只需要vmlinux更新
    
    2 ./kernel_platform/oplus/build/oplus_build.sh -t user -p SM8750 -b drivers/power/oplus/v2/oplus_chg_v2.ko ../vendor/oplus/kernel/boot/oplus_phoenix
    不需要更新vmlinux，只有KO的修改。
    oplus ddk KO编译必须完全参考kernel_platform/oplus/config/modules.ext.oplus填写target，不能随意删除路径
    in-tree KO参考kernel_platform/msm-kernel/sun.bzl->_sun_in_tree_modules = [ ]
    
    3 ./kernel_platform/oplus/build/oplus_build.sh -t user -p SM8750 -b dtbo/dtb
    所有dtb dtbo全部编译，这种会通过触发kernel编译实现，相对耗时多些
    
    4 ./kernel_platform/oplus/build/oplus_build.sh -t user -p SM8750 -b dodge-23821-sun-overlay.dtbo/sun.dtb
    清楚的知道dts修改在哪个dtb dtbo生效，可以直接指定具体dtbo、dtb编译,这种只针对kernel_platform下的，
    如果vendor/qcom下的模块自己的dts请参考5
    
    5. ./kernel_platform/oplus/build/oplus_build.sh -t user -p SM8750 -b camera-devicetree/vendor/qcom/opensource/camera-devicetree
    针对只修改了模块自己dts的场景
"
}
